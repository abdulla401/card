...
aws cloudformation create-stack \
  ...
  --parameters \
      ParameterKey=NetworkStackName,ParameterValue=card-processor-network \
      ParameterKey=ServiceName,ParameterValue=abdulla401-todo-app \
      ParameterKey=ImageUrl,ParameterValue=docker.io/abdulla401/card:latest \
      ParameterKey=ContainerPort,ParameterValue=8080

aws cloudformation wait stack-create-complete --stack-name card-processor-service

CLUSTER_NAME=$(aws cloudformation describe-stacks --stack-name card-processor-network --output text --query 'Stacks[0].Outputs[?OutputKey==`ClusterName`].OutputValue | [0]')
